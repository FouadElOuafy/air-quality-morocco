...
export default function Dashboard() {
  ...
  useEffect(() => {
    axios.get(`${API_URL}/cities`)
      .then(res => { setCityStats(res.data); setLoading(false) })
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ textAlign: "center" }}>
        <h2 style={{ fontSize: "clamp(16px, 4vw, 22px)", fontWeight: "bold", color: "#1e293b", margin: 0 }}>
        ...
        {cityStats.map(city => {
          ...
          return (
            <div key={city.city} style={{
              ...
              <p style={{ margin: 0, fontWeight: "bold", color: "#1e293b", fontSize: 14 }}>
                📍 {city.city}
              </p>
              <div style={{ fontSize: "clamp(24px, 6vw, 36px)", fontWeight: "bold",
                color: CITY_COLORS[city.city], margin: "6px 0" }}>
                {city.avg}
              </div>
              <p style={{ margin: 0, color: "#94a3b8", fontSize: 11 }}>µg/m³ moyenne</p>
              <div style={{ marginTop: 6, display: "inline-block", background: level.color,
                color: "white", borderRadius: 10, padding: "2px 10px", fontSize: 11, fontWeight: 600 }}>
                {level.label}
              </div>
              <div style={{ marginTop: 8, fontSize: 11, color: "#64748b" }}>
                Min: <b>{city.min}</b> · Max: <b>{city.max}</b>
              </div>
            </div>
          )
        })}
      ...
      <div style={{ background: "white", borderRadius: 14, padding: 20,
        ...
        <h3 style={{ margin: "0 0 16px", fontSize: 15, color: "#1e293b" }}>
          📊 Comparaison PM2.5 (Moyenne / Min / Max)
        </h3>
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={barData} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
            <XAxis dataKey="name" tick={{ fill: "#64748b", fontSize: 11 }} />
            <YAxis tick={{ fill: "#64748b", fontSize: 11 }} unit="µg" />
            <Tooltip
      ...
      <div style={{ background: "#f8fafc", borderRadius: 14, padding: 16,
        ...
        <h3 style={{ margin: "0 0 12px", fontSize: 14, color: "#1e293b" }}>💡 Interprétation</h3>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {[
            { city: "Marrakech", reason: "Proche du désert, vent faible (1.54 m/s)" },
            { city: "Casablanca", reason: "Vent fort Atlantique (5.08 m/s)" },
            { city: "Rabat", reason: "Capitale bien ventilée" },
            { city: "Fès", reason: "Ville intérieure, médina dense" }
          ].map(item => (
            <div key={item.city} style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <div style={{ width: 10, height: 10, borderRadius: "50%", flexShrink: 0,
                background: CITY_COLORS[item.city] }} />
