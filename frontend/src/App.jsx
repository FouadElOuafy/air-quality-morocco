...
export default function App() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError("Impossible de contacter l'API. Vérifiez la connexion.")
    }
    setLoading(false)
  }
    ...
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 0" }}>
          ...
          <span style={{ fontWeight: "bold", fontSize: 16, color: "#1e293b" }}>Air Quality Morocco</span>
          <span style={{ background: "#dcfce7", color: "#16a34a", fontSize: 10,
            fontWeight: 600, padding: "2px 8px", borderRadius: 12 }}>ML</span>
        </div>
          ...
          {[
            ...
            <button key={tab.id} onClick={() => setPage(tab.id)}
              style={{
                ...
                background: page === tab.id ? "#3b82f6" : "#f1f5f9",
      ...
      <div style={{ textAlign: "center", padding: "24px 16px 16px" }}>
        <h1 style={{ fontSize: "clamp(18px, 5vw, 28px)", fontWeight: "bold", color: "#1e293b", margin: 0 }}>
          {page === "predict" ? "🔍 Prédiction Qualité de l'Air" : "📊 Dashboard Analytique"}
        </h1>
        <p style={{ color: "#64748b", marginTop: 6, fontSize: "clamp(12px, 3vw, 15px)" }}>
          {page === "predict"
            ? "Entrez les paramètres météo pour prédire le PM2.5"
            : "Statistiques réelles — 5 jours via OpenWeatherMap"}
        </p>
      </div>
      ...
      <div style={{ padding: "0 16px 60px" }}>
        {page === "predict" ? (
          <div style={{
            ...
            display: "flex",
            gap: 20,
            flexDirection: "column"  // mobile: colonne par défaut
          }}>
            <style>{`
              @media (min-width: 768px) {
                .predict-layout { flex-direction: row !important; }
              }
            `}</style>
            <div className="predict-layout" style={{
              display: "flex",
